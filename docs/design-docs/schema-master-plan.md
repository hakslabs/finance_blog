...
| `macro_series`                        | catalog row per series        | `(id, code, name, source, frequency)`. Stores the _definition_ of a macro indicator.                              |
...
Fixtures will be removed _file-by-file_ in the migration-slice PRs as soon as the corresponding table can serve the same screen via the API.
