...
function fmtPrice(price: number, ticker: string) {
  return isKRTicker(ticker) ? `₩${price.toLocaleString()}` : `$${price.toFixed(2)}`;
}
...
interface Trade {
  ...
  type: "매수" | "매도"; shares: number; price: number;
  total: number; fee: number; note: string; journalLinked: boolean;
}
interface Journal {
  id: string; date: string; symbol: string; name: string;
  type: "매수" | "매도"; reason: string; target: number;
  stopLoss: number; status: "진행중" | "완료" | "손절";
  tradeId?: string; isMock: boolean;
  followUps: { date: string; note: string }[];
}
interface AlertItem {
  id: string; symbol: string; name: string;
  type: "목표가 도달" | "손절 라인" | "거래량 급증" | "뉴스 알림";
  condition: string; status: "활성" | "완료" | "비활성";
  created: string; journalId?: string;
}

// ── Initial Data ──────────────────────────────────────────────
const INIT_HOLDINGS = [
  { symbol: "NVDA", name: "엔비디아", shares: 10, avgPrice: 420.0, currentPrice: 875.4, sector: "반도체" },
  { symbol: "AAPL", name: "애플", shares: 25, avgPrice: 165.0, currentPrice: 189.3, sector: "기술" },
  { symbol: "MSFT", name: "마이크로소프트", shares: 15, avgPrice: 310.0, currentPrice: 378.9, sector: "기술" },
  { symbol: "AMZN", name: "아마존", shares: 8, avgPrice: 140.0, currentPrice: 182.1, sector: "소비재" },
  { symbol: "005930", name: "삼성전자", shares: 100, avgPrice: 68000, currentPrice: 74500, sector: "반도체" },
  { symbol: "000660", name: "SK하이닉스", shares: 50, avgPrice: 130000, currentPrice: 168000, sector: "반도체" },
];
const INIT_TRADES: Trade[] = [
  { id: "t1", date: "2026-05-15", symbol: "NVDA", name: "엔비디아", type: "매수", shares: 5, price: 875.4, total: 4377, fee: 4.38, note: "AI 수요 지속 성장 기대", journalLinked: true },
  { id: "t2", date: "2026-05-10", symbol: "AAPL", name: "애플", type: "매수", shares: 10, price: 189.3, total: 1893, fee: 1.89, note: "WWDC 기대감, 서비스 매출 성장", journalLinked: true },
  { id: "t3", date: "2026-04-28", symbol: "TSLA", name: "테슬라", type: "매도", shares: 20, price: 248.5, total: 4970, fee: 4.97, note: "목표가 도달, 차익 실현", journalLinked: false },
  { id: "t4", date: "2026-04-15", symbol: "005930", name: "삼성전자", type: "매수", shares: 50, price: 74500, total: 3725000, fee: 3725, note: "HBM 수요 증가, 반도체 사이클 상승", journalLinked: true },
  { id: "t5", date: "2026-03-20", symbol: "MSFT", name: "마이크로소프트", type: "매수", shares: 5, price: 378.9, total: 1894.5, fee: 1.89, note: "Azure AI 성장", journalLinked: false },
];
const INIT_JOURNALS: Journal[] = [
  { id: "j1", date: "2026-05-15", symbol: "NVDA", name: "엔비디아", type: "매수", reason: "AI 인프라 투자 사이클이 2-3년 지속될 것으로 판단. 데이터센터 GPU 수요는 구조적 성장. 현재 PER 40배는 성장률 대비 합리적.", target: 1000, stopLoss: 750, status: "진행중", tradeId: "t1", isMock: false, followUps: [{ date: "2026-05-16", note: "실적 발표 예정. 가이던스 확인 필요." }, { date: "2026-05-18", note: "실적 서프라이즈. 목표가 상향 검토." }] },
  { id: "j2", date: "2026-04-28", symbol: "TSLA", name: "테슬라", type: "매도", reason: "목표가 $250 도달. 경쟁 심화로 마진 압박 우려. 단기 차익 실현.", target: 250, stopLoss: 200, status: "완료", tradeId: "t3", isMock: false, followUps: [{ date: "2026-04-29", note: "매도 후 추가 상승 +3%. 아쉽지만 원칙 준수." }] },
  { id: "j3", date: "2026-05-01", symbol: "AMZN", name: "아마존", type: "매수", reason: "AWS 성장세 지속, 광고 매출 급증. 목표가 $220 설정. (모의투자 - 아직 실제 매수 미등록)", target: 220, stopLoss: 165, status: "진행중", isMock: true, followUps: [] },
];
const INIT_ALERTS: AlertItem[] = [
  { id: "a1", symbol: "NVDA", name: "엔비디아", type: "목표가 도달", condition: "≥ $1,000", status: "활성", created: "2026-05-15", journalId: "j1" },
  { id: "a2", symbol: "AAPL", name: "애플", type: "손절 라인", condition: "≤ $160", status: "활성", created: "2026-05-10" },
  { id: "a3", symbol: "005930", name: "삼성전자", type: "목표가 도달", condition: "≥ ₩90,000", status: "활성", created: "2026-04-15" },
  { id: "a4", symbol: "TSLA", name: "테슬라", type: "목표가 도달", condition: "≥ $250", status: "완료", created: "2026-04-01", journalId: "j2" },
];
...
function StockDetailModal({ ticker, onClose, onAddTrade }: { ticker: string; onClose: () => void; onAddTrade?: (ticker: string) => void }) {
  ...
  const chartPts = Array.from({ length: 30 }, (_, i) => ({
    x: i,
    y: stock.price * (0.95 + Math.sin(i * 0.4) * 0.04 + (i / 30) * 0.05 + (i % 3) * 0.01),
  }));
      ...
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-mono bg-muted/50 px-2 py-0.5 rounded text-muted-foreground">{ticker}</span>
                <span className="text-xs text-muted-foreground">{stock.exchange}</span>
              </div>
              <h2 className="text-lg font-bold font-['Outfit']">{stock.name}</h2>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-2xl font-bold font-mono">{fmtPrice(stock.price, ticker)}</span>
                <span className={cn("flex items-center gap-0.5 text-sm font-semibold", changeColor)}>
                  {stock.changePct >= 0 ? <ArrowUpRight size={14} /> : <ArrowDownRight size={14} />}
                  {Math.abs(stock.changePct).toFixed(2)}%
                </span>
              </div>
            </div>
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-muted transition-colors text-muted-foreground"><X size={15} /></button>
          </div>
        </div>
        <div className="px-5 pt-4">
          <svg width="100%" viewBox="0 0 280 65" className="overflow-visible">
            ...
            <path d={pathD + ` L 280 65 L 0 65 Z`} fill={`url(#cg-${ticker})`} />
      ...
      <div className="relative bg-card border border-border rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-xl z-10 overflow-hidden">
                  {searchResults.map(s => (
                    <button key={s.ticker} onClick={() => { setForm(f => ({ ...f, symbol: s.ticker, price: String(s.price) })); setSearch(s.ticker + " " + s.name); setShowSearch(false); setTouched(t => ({ ...t, symbol: true })); }}
                      ...
                      <span className="text-xs text-muted-foreground font-mono">{fmtPrice(s.price, s.ticker)}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1.5 block">거래 유형</label>
              <div className="flex gap-1">
                {(["매수", "매도"] as const).map(t => (
                  <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))}
                    className={cn("flex-1 py-2 rounded-lg text-sm font-medium transition-colors", form.type === t ? (t === "매수" ? "bg-up/20 text-up border border-up/40" : "bg-down/20 text-down border border-down/40") : "bg-muted/30 text-muted-foreground hover:text-foreground")}>{t}</button>
                ))}
              </div>
            </div>
            <div>
          ...
          {total > 0 && (
            <div className="bg-muted/20 rounded-lg p-3 flex items-center justify-between">
              ...
              <span className="text-sm font-bold font-mono">{fmtValue(total, form.symbol)}</span>
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} placeholder="투자 이유, 전략 등..." rows={2}
...
function PortfolioTab() {
  ...
  const totalValueUSD = holdings.reduce((s, h) => s + (h.isKR ? h.marketValue / 1300 : h.marketValue), 0);
  const totalPnlUSD = holdings.reduce((s, h) => s + (h.isKR ? h.pnl / 1300 : h.pnl), 0);
  const totalPnlP = (totalPnlUSD / (totalValueUSD - totalPnlUSD)) * 100;
  const [chartPeriod, setChartPeriod] = useState<'주' | '월' | '년'>('월');
  const sectorMap: Record<string, number> = {};
  holdings.forEach(h => { sectorMap[h.sector] = (sectorMap[h.sector] ?? 0) + (h.isKR ? h.marketValue / 1300 : h.marketValue); });
  ...
  const portfolioChartData = (() => {
    if (chartPeriod === '주') {
      ...
      return Array.from({ length: 12 }, (_, i) => {
        ...
        const base = totalValueUSD * (0.60 + i * 0.036);
        return { label, value: Math.round(base) };
      });
    }
  })();
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "총 평가금액", value: fmtUSD(totalValueUSD), sub: `≈ ${fmtKRW(totalValueUSD * 1300)}`, color: "text-foreground" },
          { label: "총 수익", value: `${totalPnlUSD >= 0 ? "+" : ""}${fmtUSD(totalPnlUSD)}`, sub: `${totalPnlP >= 0 ? "+" : ""}${totalPnlP.toFixed(2)}%`, color: totalPnlUSD >= 0 ? "text-up" : "text-down" },
          { label: "보유 종목", value: `${holdings.length}개`, sub: "미국 4 · 국내 2", color: "text-foreground" },
          { label: "이달 수익", value: "+$3,154", sub: "+8.5% MoM", color: "text-up" },
        ].map(c => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="flex gap-1">
              {(['주', '월', '년'] as const).map(p => (
                <button key={p} onClick={() => setChartPeriod(p)}
                  className={cn('text-xs px-2 py-1 rounded-lg border transition-colors', chartPeriod === p ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:text-foreground')}>{p}</button>
              ))}
            </div>
          </div>
            ...
            {portfolioChartData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                <div className="w-full relative" style={{ height: 72 }}>
                  <div className={cn('absolute bottom-0 w-full rounded-t transition-all', i === portfolioChartData.length - 1 ? 'bg-primary' : 'bg-muted/60')}
                    style={{ height: `${(d.value / maxBar) * 100}%` }} />
                </div>
                {d.label && <span className="text-[8px] text-muted-foreground truncate w-full text-center">{d.label}</span>}
              </div>
            ))}
          </div>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={100}>
            <PieChart>
              <Pie data={sectorData} cx="50%" cy="50%" innerRadius={28} outerRadius={45} dataKey="value">
          ...
          <div className="space-y-1 mt-1">
            {sectorData.map((s, i) => (
              <div key={s.name} className="flex items-center justify-between text-xs">
                ...
                <span className="font-medium">{((s.value / totalValueUSD) * 100).toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">클릭하면 상세 정보</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                {["종목", "수량", "평균단가", "현재가", "평가금액", "수익(손실)", "수익률"].map(h => (
            ...
            <tbody>
              {holdings.map(h => (
                <tr key={h.symbol} onClick={() => setSelectedTicker(h.symbol)}
                  ...
                  <td className="px-4 py-3 font-mono font-medium">{fmtPrice(h.currentPrice, h.symbol)}</td>
                  <td className="px-4 py-3 font-mono font-semibold">{fmtValue(h.marketValue, h.symbol)}</td>
                  <td className={cn("px-4 py-3 font-mono text-sm", h.pnl >= 0 ? "text-up" : "text-down")}>
                    {h.pnl >= 0 ? "+" : ""}{fmtValue(Math.abs(h.pnl), h.symbol)}
                  </td>
                  <td className={cn("px-4 py-3 font-mono font-semibold", h.pnlP >= 0 ? "text-up" : "text-down")}>
                    {h.pnlP >= 0 ? "+" : ""}{h.pnlP.toFixed(2)}%
                  </td>
                </tr>
              ))}
            </tbody>
...
function WatchlistTab() {
  ...
  const defaultList = [
    { ticker: "META", name: "메타", exchange: "NASDAQ", price: 512.3, changePct: 0.47, sector: "기술", addedAt: "" },
    { ticker: "GOOGL", name: "알파벳", exchange: "NASDAQ", price: 172.8, changePct: -0.69, sector: "기술", addedAt: "" },
    { ticker: "TSLA", name: "테슬라", exchange: "NASDAQ", price: 248.5, changePct: 3.63, sector: "자동차", addedAt: "" },
    { ticker: "035420", name: "NAVER", exchange: "KRX", price: 198500, changePct: 1.79, sector: "IT", addedAt: "" },
    { ticker: "035720", name: "카카오", exchange: "KRX", price: 42300, changePct: -1.86, sector: "IT", addedAt: "" },
  ];
  const displayList = watchlist.length > 0 ? watchlist : defaultList;
  const searchResults = useMemo(() => {
    if (!search.trim()) return [];
    const q = search.toLowerCase();
    return ALL_STOCKS.filter(s => (s.ticker.toLowerCase().includes(q) || s.name.toLowerCase().includes(q)) && !displayList.some(w => w.ticker === s.ticker)).slice(0, 8);
  }, [search, displayList]);
  useEffect(() => {
  ...
  return (
    <div className="space-y-4">
      <div ref={searchRef} className="relative">
        ...
        {showSearch && searchResults.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-xl shadow-2xl z-20 overflow-hidden">
            {searchResults.map(s => (
              <button key={s.ticker} onClick={() => { addToWatchlist({ ticker: s.ticker, name: s.name, exchange: s.exchange, price: s.price, changePct: s.changePct, sector: s.sector }); setSearch(""); setShowSearch(false); toast.success(`${s.ticker} 관심종목 추가`); }}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{s.exchange} · {s.sector}</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-mono">{fmtPrice(s.price, s.ticker)}</div>
                  <div className={cn("text-xs font-mono", s.changePct >= 0 ? "text-up" : "text-down")}>{s.changePct >= 0 ? "+" : ""}{s.changePct.toFixed(2)}%</div>
                </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h3 className="text-sm font-semibold">관심 종목 <span className="text-muted-foreground font-normal">({displayList.length})</span></h3>
          <span className="text-xs text-muted-foreground">클릭하면 상세 정보</span>
        </div>
        <table className="w-full text-sm">
          <thead>
              ...
              return (
                <tr key={stock.ticker} onClick={() => setSelectedTicker(stock.ticker)}
                  ...
                  <td className={cn("px-4 py-3 font-mono text-sm", stock.changePct >= 0 ? "text-up" : "text-down")}>
                    {stock.changePct >= 0 ? "+" : "-"}{isKR ? fmtKRW(changeAbs) : `$${changeAbs.toFixed(2)}`}
                  </td>
                  <td className={cn("px-4 py-3 font-mono font-semibold", stock.changePct >= 0 ? "text-up" : "text-down")}>
                    <span className="flex items-center gap-0.5">
                      {stock.changePct >= 0 ? <ArrowUpRight size={13} /> : <ArrowDownRight size={13} />}
                      {Math.abs(stock.changePct).toFixed(2)}%
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{stock.exchange}</td>
                  <td className="px-4 py-3">
                    <button onClick={e => { e.stopPropagation(); removeFromWatchlist(stock.ticker); toast.info(`${stock.ticker} 관심 해제`); }}
                      className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-muted transition-all text-muted-foreground hover:text-down"><X size={13} /></button>
                  </td>
                </tr>
              );
            })}
...
function TradesTab() {
  ...
  const profitData = useMemo(() => {
    ...
    const getLabel = (d: Date): string => {
      ...
      if (period === "주") { const ws = new Date(d); ws.setDate(d.getDate() - d.getDay()); return ws.toISOString().slice(0, 10); }
      if (period === "월") return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      return String(d.getFullYear());
    };
    // 현재부터 과거 N기간 라벨 목록 생성
    const N = period === "일" ? 14 : period === "주" ? 12 : period === "월" ? 12 : 5;
    const labels: string[] = [];
    for (let i = N - 1; i >= 0; i--) {
      const d = new Date(now);
      if (period === "일") d.setDate(d.getDate() - i);
      else if (period === "주") { d.setDate(d.getDate() - d.getDay() - i * 7); }
    ...
    [...trades].sort((a, b) => a.date.localeCompare(b.date)).forEach(t => {
      ...
      if (t.type === "매수") {
        ...
        const avg = avgMap[t.symbol] ? avgMap[t.symbol].totalCost / avgMap[t.symbol].shares : t.price;
    ...
    return labels.map(lbl => {
      ...
      const dispLabel = period === "일" ? lbl.slice(5) : period === "월" ? lbl.slice(5) + "월" : period === "주" ? lbl.slice(5) : lbl;
      return { label: dispLabel, pnl: pnlByLabel[lbl] ?? 0, cumPnl: cum };
    });
  }, [trades, period]);
  const totalRealizedPnl = profitData.length > 0 ? profitData[profitData.length - 1].cumPnl : 0;
  const maxProfit = Math.max(...profitData.map(d => Math.abs(d.pnl)), 1);
  const totalBuy = trades.filter(t => t.type === "매수").reduce((s, t) => s + t.total, 0);
  const totalSell = trades.filter(t => t.type === "매도").reduce((s, t) => s + t.total, 0);
  const totalFee = trades.reduce((s, t) => s + t.fee, 0);
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "실현손익 누적", value: `${totalRealizedPnl >= 0 ? "+" : ""}${fmtUSD(totalRealizedPnl)}`, sub: `${totalRealizedPnl >= 0 ? "+" : ""}${fmtKRW(totalRealizedPnl * 1300)}`, color: totalRealizedPnl >= 0 ? "text-up" : "text-down" },
          { label: "종 매수금액", value: fmtKRW(totalBuy), sub: fmtUSD(totalBuy / 1300), color: "text-foreground" },
          { label: "종 매도금액", value: fmtKRW(totalSell), sub: fmtUSD(totalSell / 1300), color: "text-foreground" },
          { label: "종 수수료", value: fmtKRW(totalFee), sub: fmtUSD(totalFee / 1300), color: "text-muted-foreground" },
        ].map(c => (
          <div key={c.label} className="bg-card border border-border rounded-xl p-4">
            <div className="text-xs text-muted-foreground mb-1">{c.label}</div>
            <div className={cn("text-base font-bold font-mono", c.color)}>{c.value}</div>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">현재 기준 과거 {period === "일" ? "14일" : period === "주" ? "12주" : period === "월" ? "12개월" : "5년"}</p>
          </div>
          <div className="flex gap-1">
            {(["일", "주", "월", "년"] as const).map(p => (
              <button key={p} onClick={() => setPeriod(p)}
                className={cn("text-xs px-2.5 py-1 rounded-lg transition-colors", period === p ? "bg-primary text-primary-foreground" : "bg-muted/40 text-muted-foreground hover:text-foreground")}>{p}</button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-2 justify-end">
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <div className="w-4 h-0.5 rounded" style={{ background: "#6366f1", borderTop: "2px dashed #6366f1" }} /> 기간별 실현손익
          </div>
        </div>
      </div>
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h3 className="text-sm font-semibold">거래 내역 <span className="text-muted-foreground font-normal">({trades.length}건)</span></h3>
          <button onClick={() => setShowAddModal(true)} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity">
            <Plus size={12} /> 거래 추가
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                {["날짜", "종목", "유형", "수량", "단가", "총액", "수수료", "메모"].map(h => (
                  <th key={h} className="text-left px-4 py-2.5 text-xs text-muted-foreground font-medium whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {trades.map(trade => (
                <tr key={trade.id} onClick={() => setSelectedTicker(trade.symbol)}
                  className="border-b border-border/50 hover:bg-muted/10 transition-colors cursor-pointer group">
                  <td className="px-4 py-3 text-xs text-muted-foreground font-mono">{trade.date}</td>
                  <td className="px-4 py-3">
                    <div className="font-semibold group-hover:text-primary transition-colors">{trade.symbol}</div>
                    <div className="text-xs text-muted-foreground">{trade.name}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={cn("text-xs px-1.5 py-0.5 rounded border font-semibold", trade.type === "매수" ? "border-up/40 text-up bg-up/10" : "border-down/40 text-down bg-down/10")}>{trade.type}</span>
                  </td>
                  <td className="px-4 py-3 font-mono">{trade.shares.toLocaleString()}</td>
                  <td className="px-4 py-3 font-mono text-muted-foreground">{fmtPrice(trade.price, trade.symbol)}</td>
                  <td className="px-4 py-3 font-mono font-semibold">{fmtValue(trade.total, trade.symbol)}</td>
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{fmtValue(trade.fee, trade.symbol)}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground max-w-[120px] truncate">{trade.note}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="space-y-4">
      {unlinkedTrades.length > 0 && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-amber-400 mb-1">거래내역 미연동 {unlinkedTrades.length}건 — 모의투자로 처리</div>
            <div className="text-xs text-muted-foreground">{unlinkedTrades.map(t => `${t.symbol} (${t.type})`).join(", ")} — 투자일지가 없습니다.</div>
          </div>
        </div>
      )}
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">거래 기반 투자 일지 · 후속 메모 관리</p>
        <button onClick={() => toast.info("거래 추가 시 자동 생성 옵션을 사용하세요.")} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-primary text-primary-foreground hover:opacity-90">
          <Plus size={12} /> 일지 작성
        </button>
      </div>
      <div className="space-y-3">
        {journals.map(j => (
          <div key={j.id} className={cn("bg-card border rounded-xl overflow-hidden", j.isMock ? "border-amber-500/30" : "border-border")}>
            <div className="p-4 cursor-pointer" onClick={() => setExpandedId(expandedId === j.id ? null : j.id)}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className="text-xs font-mono text-muted-foreground">{j.date}</span>
                    <span className="font-semibold">{j.symbol}</span>
                    <span className="text-xs text-muted-foreground">{j.name}</span>
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border font-semibold", j.type === "매수" ? "border-up/40 text-up bg-up/10" : "border-down/40 text-down bg-down/10")}>{j.type}</span>
                    {j.isMock && <span className="text-[10px] px-1.5 py-0.5 rounded border border-amber-500/40 text-amber-400 bg-amber-500/10 font-semibold">모의투자</span>}
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border", j.status === "진행중" ? "border-primary/40 text-primary bg-primary/10" : j.status === "완료" ? "border-up/40 text-up bg-up/10" : "border-down/40 text-down bg-down/10")}>{j.status}</span>
                  </div>
                  <p className="text-sm text-foreground/80 line-clamp-2">{j.reason}</p>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">목표가</div>
                    <div className="text-sm font-mono font-semibold text-up">{fmtPrice(j.target, j.symbol)}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground">손절가</div>
                    <div className="text-sm font-mono font-semibold text-down">{fmtPrice(j.stopLoss, j.symbol)}</div>
                  </div>
                  {expandedId === j.id ? <ChevronUp size={14} className="text-muted-foreground" /> : <ChevronDown size={14} className="text-muted-foreground" />}
                </div>
              </div>
            </div>
            {expandedId === j.id && (
              <div className="border-t border-border/50 p-4 space-y-3">
                    ...
                    <div className="space-y-2">
                      {j.followUps.map((fu, i) => (
                        <div key={i} className="flex items-start gap-2 text-sm">
                          <span className="text-xs text-muted-foreground font-mono flex-shrink-0 mt-0.5">{fu.date}</span>
                ...
                <div className="flex gap-2">
                  <input value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="후속 메모 추가..." onKeyDown={e => e.key === "Enter" && addFollowUp(j.id)}
                    className="flex-1 px-3 py-2 text-sm bg-muted/30 border border-border rounded-lg focus:outline-none focus:border-primary" />
                  <button onClick={() => addFollowUp(j.id)} className="px-3 py-2 rounded-lg bg-primary/10 text-primary text-xs hover:bg-primary/20">추가</button>
                </div>
              </div>
            )}
          </div>
...
function AlertsTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">종목 알람 설정 · 투자일지 기반 자동 등록</p>
        <button onClick={() => setShowAddModal(true)} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-primary text-primary-foreground hover:opacity-90">
          <Plus size={12} /> 알람 추가
        </button>
      </div>
      {activeJournals.length > 0 && (
        <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
          <div className="text-xs font-semibold text-primary mb-2">💡 투자일지 기반 알람 추천</div>
      ...
      <div className="space-y-2">
        {alerts.map(alert => (
          <div key={alert.id} className={cn("bg-card border border-border rounded-xl p-4 flex items-center gap-4", alert.status === "완료" && "opacity-60")}>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-muted/30 flex-shrink-0">{TYPE_ICONS[alert.type]}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {alert.journalId && <span className="text-[10px] px-1 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">일지 연동</span>}
              </div>
      ...
      {showAddModal && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl w-full max-w-sm p-5 shadow-2xl space-y-4">
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">종목 티커</label>
              <input value={form.symbol} onChange={e => setForm(f => ({ ...f, symbol: e.target.value.toUpperCase() }))} placeholder="NVDA, 005930..."
                className="w-full px-3 py-2 text-sm bg-muted/30 border border-border rounded-lg focus:outline-none focus:border-primary" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">알람 유형</label>
              <div className="grid grid-cols-2 gap-1.5">
                {(["목표가 도달", "손절 라인", "거래량 급증", "뉴스 알림"] as const).map(t => (
                  <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))}
                    className={cn("text-xs px-2 py-1.5 rounded-lg border transition-colors", form.type === t ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:text-foreground")}>{t}</button>
                ))}
              </div>
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">조건</label>
              <input value={form.condition} onChange={e => setForm(f => ({ ...f, condition: e.target.value }))} placeholder="≥ $1,000 또는 ≤ ₩70,000..."
                className="w-full px-3 py-2 text-sm bg-muted/30 border border-border rounded-lg focus:outline-none focus:border-primary" />
            </div>
            <div className="flex gap-2">
              <button onClick={() => setShowAddModal(false)} className="flex-1 py-2 rounded-lg bg-muted/40 text-sm text-muted-foreground">취소</button>
              <button onClick={handleAdd} className="flex-1 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">등록</button>
            </div>
          </div>
        </div>
        </ModalPortal>
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">포트폴리오 · 관심종목 · 거래내역 · 투자일지 · 알람</p>
      </div>
