...
> 💡 **핵심 포인트**: 주식 투자는 기업의 일부를 소유하는 것입니다. 기업이 성장하면 주가도 오르고, 배당도 늘어납니다.`,
    quiz: [
      {
        question: "주식 투자로 얻을 수 있는 수익의 두 가지 원천은?",
        options: [
          "이자와 원금",
          "자본이득과 배당",
          "임대료와 로열티",
  ...
  },
  {
    ...
    category: "투자 기초",
    title: "증권 거래소와 시장 구조",
    description: "NYSE, NASDAQ, KRX 등 주요 거래소와 시장 구조를 알아봅니다.",
    duration: "10분",
    level: "입문",
    completed: true,
...
- **한국**: 09:00~15:30 (정규장), 08:00~09:00 (프리마켓)`,
    quiz: [
      {
        question: "기술주가 많이 상장된 미국 거래소는?",
        options: ["NYSE", "NASDAQ", "KRX", "TSE"],
        answer: 1,
      },
    ],
  },
...
> 💡 **실전 팁**: 스탑로스는 감정적 매매를 방지하는 가장 효과적인 도구입니다.`,
    quiz: [
      {
        question: "원하는 가격을 지정하여 주문하는 방식은?",
        options: ["시장가 주문", "지정가 주문", "스탑로스 주문", "OCO 주문"],
        answer: 1,
      },
      {
        question: "스탑로스 주문의 주요 목적은?",
        options: ["수익 극대화", "손실 제한", "거래량 증가", "세금 절약"],
        answer: 1,
      },
    ],
  },
...
> 💡 **실전 팁**: 단일 캔들보다 2-3개의 캔들 패턴 조합이 더 신뢰도가 높습니다.`,
    quiz: [
      ...
      {
        question: "하락 후 반등 신호를 나타내는 캔들 패턴은?",
        options: ["도지", "망치형", "역망치형", "음봉"],
        answer: 1,
      },
    ],
  },
...
> 💡 **실전 팁**: 200일선은 기관투자자들이 가장 중요하게 보는 지표입니다. 200일선 위에 있으면 강세장, 아래면 약세장으로 판단합니다.`,
  ...
  // 가치투자
  {
    id: "v1",
    category: "가치투자",
    title: "PER, PBR, ROE 완전 정복",
    description: "가장 많이 쓰이는 밸류에이션 지표를 쉽게 이해하기.",
    duration: "10분",
    level: "초급",
...
> 💡 **버핏의 기준**: ROE 15% 이상, PER 15배 이하, PBR 1.5배 이하면 관심 종목으로 고려.`,
  ...
  // 퀀트
  {
    ...
    category: "퀀트 전략",
    title: "팩터 투자 입문",
    description: "학술적으로 검증된 팩터 투자 전략의 기초.",
    duration: "16분",
    level: "중급",
    content: `## 팩터 투자 입문
...
> 💡 **실전 팁**: 팩터는 시장 사이클에 따라 성과가 다릅니다. 경기 상승기에는 모멘텀, 하락기에는 퀄리티와 저변동성이 유리합니다.`,
    quiz: [
      {
        question: "모멘텀 팩터를 측정하는 주요 지표는?",
        options: ["PER, PBR", "12-1개월 수익률", "ROE, 부채비율", "시가총액"],
        answer: 1,
      },
    ],
  },
  ...
  const onSubmit = () => {
    ...
    if (user) {
      lessonsService
        ...
        .catch(() => {
          /* swallow — UI already shows local score */
        });
    }
  };
  return (
      ...
      {lines.map((line, i) => {
        if (line.startsWith("## "))
          return (
            <h2
              key={i}
              className="text-xl font-bold font-['Outfit'] mt-4 mb-2 text-foreground"
            >
              {line.slice(3)}
            </h2>
          );
        ...
        if (line.startsWith("> "))
          return (
            <blockquote
              key={i}
              className="border-l-2 border-primary pl-3 py-1 bg-primary/5 rounded-r-lg text-muted-foreground italic"
            >
              {line.slice(2)}
            </blockquote>
          );
        if (line.startsWith("| ")) {
          ...
          return (
            <div
              key={i}
              className={cn(
                "grid gap-2 text-xs py-1.5 px-2 rounded",
                isHeader ? "font-semibold bg-muted/30" : "hover:bg-muted/10",
              )}
...
export default function LearnDetail() {
  ...
  const [activeSection, setActiveSection] = useState<"content" | "quiz">(
    "content",
  );
  ...
  const lesson: Lesson | undefined =
    ...
    (liveLesson
      ? {
          ...
          duration:
            ((liveLesson as any).duration ?? (liveLesson as any).read_time)
              ? `${(liveLesson as any).read_time}분`
              : "—",
      ...
      : undefined);
  const categoryLessons = lesson
    ? ALL_LESSONS.filter((l) => l.category === lesson.category)
    : [];
  ...
  const nextLesson =
    currentIdx < categoryLessons.length - 1
      ? categoryLessons[currentIdx + 1]
      : null;
        ...
        <summary className="px-4 py-3 cursor-pointer list-none flex items-center gap-2.5 select-none">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px] text-muted-foreground">
              {categoryLessons.length}개 강의 · {currentIdx + 1}번째
            </div>
          </div>
          <ChevronRight
            size={14}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  ...
                  {lesson.popular && (
                    <Badge
                      variant="outline"
                      className="text-[10px] px-1.5 py-0 border-amber-500 text-amber-400 gap-0.5"
                    >
                ...
                <h1 className="text-xl font-bold font-['Outfit'] leading-snug">
                  {lesson.title}
                </h1>
        ...
        <div className="flex gap-1 bg-muted/40 p-1 rounded-xl">
          {[
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 text-sm px-3 py-2 rounded-lg transition-all font-medium",
