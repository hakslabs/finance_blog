...
export function FollowProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncAdd = (id: string) => { if (user && isServerId(id)) followsService.add(id).catch(() => {}); };
  const syncRemove = (id: string) => { if (user && isServerId(id)) followsService.remove(id).catch(() => {}); };

  const toggleFollow = useCallback((id: string, _name: string) => {
    setFollowedIds(prev => {
      const next = new Set(prev);
      if (next.has(id)) { next.delete(id); syncRemove(id); }
      else { next.add(id); syncAdd(id); }
      return next;
    });
