...
export const lessonsService = {
  ...
  get: (id: string) =>
    apiGet<LearnGuide & { content?: string; quiz?: unknown[] }>(
  ...
  setCompleted: (lessonId: string, completed: boolean) =>
    apiPut<LessonProgress>(
      `/me/lesson-progress/${encodeURIComponent(lessonId)}`,
      { completed },
    ),
  submitQuiz: (
    lessonId: string,
    ...
    apiPost<{ id: string; score: number }>(
      `/me/lessons/${encodeURIComponent(lessonId)}/quiz`,
      body,
    ),
};
